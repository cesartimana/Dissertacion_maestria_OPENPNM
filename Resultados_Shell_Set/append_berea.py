...
resolution = 5.345e-6 #For berea : 5.345e-6 / For carbonate: 2.85e-6
...
plt.xlabel('Shape factor $G$')
plt.ylabel('Cum. rel. freq.')
plt.xlim([10**xmin, 10**xmax])
plt.ylim([0, 1])
plt.xscale('log')
plt.xticks(np.logspace(xmin,xmax,xmax-xmin+1))
plt.legend(['Berea data old' , 'Berea data new', '$G_{max}$ for triangle', '$G_{max}$ for rectangle', '$G_{max}$ for ellipse'], fontsize="14")
plt.savefig(fname = 'Berea_G_'+net_type+'.pdf', format = 'pdf', pad_inches=0)
#-------------------------------------

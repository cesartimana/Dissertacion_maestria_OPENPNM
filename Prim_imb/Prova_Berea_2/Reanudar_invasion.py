...
with open('imbibition_process_24_07_25.pkl', 'rb') as fp:
    info_imbibition = pickle.load(fp)

s_inv = int(list(info_imbibition)[-1][7:]) #Picking last info and transform in int
#Setting a fixed status to start
s_inv = 462
...
print(status_str)
print(p_inv)
print(info_imbibition[status_str]['invaded element'])
print(info_imbibition[status_str]['type of invasion'])
...
print(p_next)
print(info_imbibition['status ' + str(s_inv)]['invasion pressure'])
print(pos)

#lineas simples para saber si hay esquinas con agua en todos los poros. Es cierto.
for item in elements:
    print('Do All ' + item + 's has water at leats in one corner?')
    print(np.all (np.any(copy_cluster_info[item + '.corner'] == 0) ) )
    print('--------')
...
while s_inv < 15000:
    ...
    #ALERTA MODIFICAR PARA QUE S_INV CAMBIE SOLO CUANDO LA PRESION SIGUIENTE ES MÁS BAJA
    s_inv +=1

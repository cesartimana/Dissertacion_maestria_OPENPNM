                ...
                if displacement == 'max_LC':
                    ...
                    for i in range(3):
                        if mask_LC[i]:
                            ...
                            #Getting the nwp and wp index
                            old_index = copy_cluster_info[item + '.layer'][pos, i]
                            wp_index = copy_cluster_info[item + '.corner'][pos, i]
                            #Update phase location
                            copy_invasion_info[item + '.layer'][pos, i] = True
                            copy_cluster_info[item + '.layer'][pos, i] = wp_index
                            #Setting pc_LC as -inf
                            pc_dict[f'{item}.layer_collapse'][pos, i] =  -np.inf
                    wp_ce_index = copy_cluster_info[item + '.center'][pos]
                    mask_LF =  ~copy_invasion_info[item + '.layer'][pos, :]
                    if wp_index != wp_ce_index:
                        #Corner and center clusters were different. Now are merged
                        copy_cluster_info = _if.merge_cluster(copy_cluster_info, [wp_index, wp_ce_index], layer = True)
                else:
                    ...
                    if displacement == 'snapoff':
                        ...
                        if len(wp_item_index_list) > 1:
                            bool_merge_wp = True
                    #Setting wp index on the wp locations #SI MTM CENTER Y CORNER SON ESPERADOS, ESTO NO VA
                    for loc in locations:
                    ...
                    if bool_merge_wp:
                        ...
                        if bool_divided:
                            ...
                            raise Exception('Parece que aun hay mas de un cluster de wp luego de unirseles')

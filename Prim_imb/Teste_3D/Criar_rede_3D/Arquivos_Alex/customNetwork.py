...
D_t = np.float_power(permeability*A_in*128/N_tub/z_mean*np.pi, 0.25)
...
D_p_dev = np.min([(sp-D_p)/D_p, 0.3])
print('D p loc', D_p_dev)

pn = op.network.Cubic(shape=shape, spacing=sp, connectivity=26)

pore_seeds = op.models.geometry.pore_seed.random
pn.add_model(propname='pore.seed', model=pore_seeds, num_range=[0.01, 0.99])
inds = np.where(pn['pore.seed'] > Np_th)
op.topotools.trim(pn, pores=inds)
pn.remove_model('pore.seed')
op.topotools.reduce_coordination(network=pn, z=z_mean)
plt.hist(pn.num_neighbors(pn.Ps), bins=np.arange(1,21), align='left', density=True)
plt.show()
...
def min_permeability_err(x):
    ...
    geo.models['throat.diameter']['scale'] = np.abs(x)*0.2
...
def min_porosity_err(y):
    ...
    D_p_dev = np.max([0.01, np.min([(sp-y)/y, 0.15])])
...
for i in range(3):
    ...
    timeZero = time.time()
    sol = optimize.root_scalar(min_porosity_err, x0=D_p, rtol=1e-4, bracket=[D_p*0.001, sp*0.99], maxiter=20)
...
#op.io.VTK.save(network=pn, filename= 'customNetwork')

...
porosity_final, _ = calc_porosity(min_seed = range_Dp[0], max_seed = range_Dp[1])
...
min_value= range_Dp[0]
max_value = range_Dp[1]
i = 0
while error > tol:
    range_tune = (min_value + max_value)/2
    if mode == 'mod_min':
        porosity_final, Dp_eq = calc_porosity(min_seed = range_tune, max_seed = range_Dp[1])
    elif mode == 'mod_max':
        porosity_final, Dp_eq = calc_porosity(min_seed = range_Dp[0], max_seed = range_tune)
    error = np.abs(porosity_final - porosity) / porosity
    if porosity_final > porosity:

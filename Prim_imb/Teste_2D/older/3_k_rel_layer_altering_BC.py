...
for item in elements:
    ...
    flow_results[f'{item}.pc_theta_a'] = pmax_drn * np.cos(theta_a + beta) / np.cos(theta_r + beta)
    ...
    for item in elements:
        ...
        for i in range(N):
            ...
            if status_info['cluster_center'][i] in trapped_list:
                ...
                posible_cluster = status_info['cluster_layer'][i,:]
                nw_cluster = np.unique(posible_cluster[posible_cluster != 0])
                if len(nw_cluster) > 0:
                    if nw_cluster in trapped_list:
                        pos = np.where(nw_cluster == trapped_list)[0]
                        bool_trap = True
            if bool_trap:
        ...
        if wp:
                ...
                if pn['throat.boundary'][t]:
                    ...
                    for item in elements:
                        if item == 'pore':
                            ...
                            n = t
                        for loc in locations:
                            info_imbibition[status_str]['invasion_info'][f'{item}.{loc}'][n] = status
                            info_imbibition[status_str]['cluster_info'][f'{item}.{loc}'][n] = index
        else:
            phase = oil
            ...
            for t in range(Nt):
                if pn['throat.boundary'][t]:
                    ...
                    #Modifying boundary elements, center, layer and corner phases
                    for loc in locations:
                        status = info_imbibition[status_str]['invasion_info'][f'pore.{loc}'][p_int]
                        index = info_imbibition[status_str]['cluster_info'][f'pore.{loc}'][p_int]
                        for item in elements:
                            if item == 'pore':
                                n = p_b
                            else:
        ...
        for item in elements:
            ...
            if item == 'pore':
                continuity_list = _if.identify_continuous_cluster(cluster_pore_center = status_info[f'{item}.cluster_center'],
                                                                  ...
                                                                  cluster_pore_layer = status_info[f'{item}.cluster_layer'],
                                                                  layer = True)
                cluster_list = []
                for loc in locations:
...
for s_inv in status_array:
    status_str = 'status ' + str(s_inv)
    sat.append(geometric_data[status_str]['wetting_phase_saturation'])
...
np.save('results_K_Pc_layer_altering_BC', output) #If wp, then 0. Otherwise 1
np.savetxt('results_K_Pc_layer_altering_BC.txt', output, fmt=' %.5e '+' %.5e '+' %.5e ' + ' %.5e ', header=' sat // krel_w// krel_o // pc')

...
class Primary_Drainage(Algorithm):
    ...
    def run(self, throat_diameter = 'throat.diameter', ignore_boundary = False, pc_max = np.inf):
        r"""
    ...
    def _run_algorithm(self, wp_dict, cluster_dict, pc_max = np.inf):
        ...
        while p_next <= pc_max:
            ...
            save_last_status = False
            if s_inv > 10000:
                print('Forced to end invasion after 10000 equilibriom status.')
                break
            if np.isinf(p_next):
            ...
            if p_next > pc_max:
                ...
                save_last_status = True
            if save_last_status:
                #Last quasi-static state
                s_inv +=1
                status_str = 'status ' + str(s_inv)
                previous_status = 'status ' + str(s_inv - 1)
                p_inv = p_next
                info_drainage[status_str] = copy.deepcopy(info_drainage[previous_status])
                info_drainage[status_str]['invasion pressure'] = pc_max
                break

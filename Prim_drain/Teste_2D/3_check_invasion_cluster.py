...
#Boundary elements
_ = op.visualization.plot_connections(network=pn, throats=pn.throats('boundary'), ax=ax4, c = 'r', linewidth = 11, zorder = 0)
_ = op.visualization.plot_coordinates(network=pn, pores=pn.pores('boundary'), c='r', s=600, ax = ax4, zorder = 0)
plot_color_clusters(cl_index = cluster_index, color = 'b', ax = ax4)
ax4.set_title(f'Cluster {cluster_index}', fontsize = 20)

plt.tight_layout()
#print(check_cluster['invasion_info']['throat.center'][105])
#print(check_cluster['cluster_info']['throat.center'][105])
plt.show()

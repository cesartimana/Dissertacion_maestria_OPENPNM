...
offset = [0, -10.0e-05, 0]
#op.topotools.add_boundary_pores(pn, pn.pores('front'), offset)
pn.add_boundary_pores(labels = 'front', spacing = offset)

print(pn)

#Set boundary pores
inlet_pores = pn.pores('front_boundary')
outlet_pores = pn.pores('back')

pn['throat.diameter'][-10:] = pn['throat.diameter'][np.arange(0,82,9)]
pn['pore.diameter'][-10:] = pn['pore.diameter'][pn['pore.front']]
...
path = np.array([183,27,28,29,30,31,32,33,34,35, #main branch
...
pn['throat.diameter'][30] = np.minimum(D1[30], D2[30]) * 0.7 #Connection of third branch with a lower diameter than other throats from path
...
inv_pattern = pd['throat.invasion_sequence'] <7
print(max(pd['throat.invasion_pressure'][inv_pattern]))
ax = op.visualization.plot_coordinates(network=pn, pores=pn.pores('front_boundary'), c='r', s=50)
ax = op.visualization.plot_coordinates(network=pn, pores=pn.pores('front_boundary', mode='not'), c='grey', ax=ax)

...
def conductance(
    ...
    #Center Conductance
    cond_center = _np.ones_like(status_center) * null_value
...
def conduit_lenght_tubes(
    ...
    Lt = _np.maximum(L_ctc - (L1 + L2), min_L)
...
def conduit_conductance_2phases(
    ...
    if corner:
        ...
        pore_g_co = _np.ones_like(pore_g_ce) * null_g
        throat_g_co = _np.ones_like(throat_g_ce) * null_g

    #Layers can only exist if corner is True
    if layer and not corner:
        raise Exception('Layers can only exist if corner is True')
    elif not layer:
        pore_g_la = _np.ones_like(pore_g_ce) * null_g
        throat_g_la = _np.ones_like(throat_g_ce) * null_g
    ...
    P12_g = _np.vstack((P1_g , throat_g, P2_g)).T
    g_L = _np.sum(conduit_length / P12_g, axis = 1 )**(-1)
    return g_L

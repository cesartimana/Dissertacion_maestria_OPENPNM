...
inv_pattern = pd['throat.invasion_sequence'] < 55
inv_pattern[[90, 100]] = True
...
#Asi funciona bond percolation
# poro 0 abajo izquierda, y conteo hacia arriba
import scipy.sparse as sprs
from scipy.sparse import csgraph
conns = pn['throat.conns']
occupied_bonds = inv_pattern

#occupied_sites contiene como True a los poros vecinos de las gargantas invadidas
Np = np.amax(conns) + 1
occupied_sites = np.zeros([Np, ], dtype=bool)
occupied_sites[conns[occupied_bonds].flatten()] = True


adj_mat = sprs.csr_matrix((occupied_bonds.astype(int), (conns[:, 0], conns[:, 1])), shape=(Np, Np))
adj_mat.eliminate_zeros()
#Identifica los items conectados y asigna un numero. Gargantas conectadas tienen el mismo numero

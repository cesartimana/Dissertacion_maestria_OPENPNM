...
@author: cesar
SOLO PARA VER SI DA EL MISMO RESULTADO QUE USANDO MIS FUNCIONES

A PESAR QUE IGNORE EL FACTOR F, EL RESULTADO SALIO SIMILAR A MIS CODIGOS
...
Ap =  pn['pore.cross_sectional_area'][conns]
Lp = np.delete(L, 1, axis = 1)
HSF_p_prism= 0.6 * Ap ** 2 * 3**0.5 / 36 / Lp #Notese que ignore F
HSF_p_prism[mask_pBC] = np.inf
pn['throat.hydraulic_size_factors'][:, 0] = HSF_p_prism[:, 0]
pn['throat.hydraulic_size_factors'][:, 2] = HSF_p_prism[:, 1]

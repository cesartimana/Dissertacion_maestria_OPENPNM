...
print(hull_in.volume)
print(hull_out.volume)
...
#Cambiando el area de los poros, considerando prisma con L=D_pores_inscribed
pn['pore.cross_sectional_area'] = pn['pore.volume'] / pn['pore.inscribed_diameter']
...
for wp in [True,False]:
    ...
    for item in elements:
        ...
        if item == 'pore':
            pore_g_center, pore_g_corner, _, _ = _cf.conductance(pn, status_center, status_corner, theta_corner, bi_corner, viscosity, item = item, one_phase_corr = True)
            pore_g_center = np.ones_like(pore_g_center)
            print('----pore------')
            print(pore_g_center)
            print(np.average(pore_g_center))
        else:
            throat_g_center, throat_g_corner,_,_= _cf.conductance(pn, status_center, status_corner, theta_corner, bi_corner, viscosity, item = item, one_phase_corr = True)
            print('----throat----')
            #print(throat_g_center)
            #print(np.average(throat_g_center))
    ...
    sph_g_L = _cf.conduit_conductance_2phases(network = pn,
                                            ...
                                            conduit_length = L,
                                            pore_g_co = pore_g_corner,
                                            throat_g_co = throat_g_corner,
                                            pore_g_la = None,
                                            throat_g_la = None,
                                            corner = False,
                                            layer = False)
